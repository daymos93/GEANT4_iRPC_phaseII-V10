...
// RunManager includes (depending on if ST or MT Geant4 build is used)
...
int main(int argc, char** argv) {
    ...
    //
    // Construct the default run manager (depending on if ST or MT Geant4 build is used)
#ifdef G4MULTITHREADED
    G4MTRunManager* runManager = new G4MTRunManager;
    // Set number of threads if you want:
//    G4int nThreads = std::min(G4Threading::G4GetNumberOfCores(), 4);
//    runManager->SetNumberOfThreads(nThreads);
    // NOTE: the same can be achived by the  /run/numberOfThreads 4 UI command
    //       that must be before /run/initialize
#else
    ...
    runManager->SetUserInitialization( new GIFppActionInitialization( detectorConstruction ) );
